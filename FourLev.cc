...
Block* FourLev::NextBlock(int loc){
    ...
    if (this->random==false){
        ...
        this->blocks.clear();
        this->blocks = {'S','S','Z','Z','I','J','L','O','T'};
        k = rand() % (this->blocks.size());
    }
    if ((this->same > 0) && (this->same%5 == 0)) return new xblock();
    else if (this->blocks[k] == 'I') return new iblock{true};
        else if (this->blocks[k] == 'J') return new jblock{true};
        else if (this->blocks[k] == 'L') return new lblock{true};
        else if (this->blocks[k] == 'O') return new oblock{true};
        else if (this->blocks[k] == 'S') return new sblock{true};
        else if (this->blocks[k] == 'Z') return new zblock{true};
